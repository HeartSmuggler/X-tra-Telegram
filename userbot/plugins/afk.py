...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = f"__HEART SMUGGLER Has Been Gone For__ `{total_afk_time}`\nWhere He Is: ~~ONLY GOD KNOWS~~ " + \
            f"\n\n____\n**REASON**: {reason}" \
            if reason \
            else f"**Hei Whatsup?**\n__I am currently offline. Since when, you ask? For {total_afk_time} I guess.__\n\nCatch you soon ~~Soon~~ __All Is Well__**( ಠ ʖ̯ ಠ)**  "
